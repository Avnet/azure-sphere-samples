...
// Variables used to update the polling time between reading the attached sensors
EventLoopTimer *telemetrytxIntervalr = NULL;

// Variables used to update the polling time between reading sensor data
...
///     
///     This handler is responsible for reading the connected sensor data into global variables.
//      The SendTelemetryTimerEventHandler() handler is responsible for sending telemetry.  This
///     allows the application to poll the sensors quickly, maybe looking for an anomaly in the data.
/// </summary>
static void ReadSensorTimerEventHandler(EventLoopTimer *timer)
{
    ...
    if(firstPass){
        ...
        // Set the OLED screen to the Avnet Logo
        oled_state = LOGO;
        firstPass = false;
    }
#endif 
...
#ifdef M4_INTERCORE_COMMS
    // Request sensor data from the real time applications.
    RequestRawData();
#endif     
    ...
    if(IoTCConnected){
        ...
        if(!firstPass){
            ...
            snprintf(pjsonBuffer, JSON_BUFFER_SIZE,
                ...
                "%.2f, \"aZ\": %.2f, \"pressure\": %.2f, \"rssi\": %d}",
                acceleration_g.x, acceleration_g.y, acceleration_g.z, angular_rate_dps.x,
                angular_rate_dps.y, angular_rate_dps.z, pressure_kPa, network_data.rssi);
                                     ...
                                     void *userContextCallback)
{
	...
	checkAndUpdateDeviceTwin("versionString", "AvnetSK-AD-V1.1", TYPE_STRING, false, true);
...
static void ReadWifiConfig(bool outputDebug){
	...
	else 
	{
        ...
        if((iothubClientHandle != NULL) && (ssidChanged)){
   			...
   			checkAndUpdateDeviceTwin("ssid", &network_data.SSID, TYPE_STRING, false, true);
   			checkAndUpdateDeviceTwin("freq", &network_data.frequency_MHz, TYPE_INT, false, true);
            uint16_t wifiChannel = get_channel((uint16_t)network_data.frequency_MHz);
            checkAndUpdateDeviceTwin("wifiChannel", &wifiChannel, TYPE_INT, false, true);
            checkAndUpdateDeviceTwin("bssid", &bssid, TYPE_STRING, false, true);
